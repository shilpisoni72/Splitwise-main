...
class App extends Component{
    constructor(){
        ...
        this.user  =    { userId:null, userName: '', noOfFriends:0, profile: 0};
        this.account =  { totalBalance: null, youOwe: null, youAreOwed: null};

        this.state =    { page:1 , signupLogin: 0, loiggedIn: false, err: 0, errorOpen: false,
    ...
    componentWillMount(){
        var cookie_User   = this.getCookie("userName");
        var cookie_LoggedIn = this.getCookie("userLoggedIn");
        var token = this.getCookie("token");
        var cookie_Id = this.getCookie("userId");
        if(cookie_LoggedIn){
            this.setState({loogedIn:true});//that.state.auth = true;
            this.user.userName = cookie_User;
            this.user.userId = cookie_Id;//console.log(this.user);
    ...
    login = (authUser) => {        
        ...
        .then(function(result) {
            ...
            if(result.code === "invalid-creds"){
                ...
                that.setCookie("userLoggedIn",true,1);
                that.setCookie("HASURA_AUTH_TOKEN",authToken,1);
                that.setCookie("userId",userId,1);
